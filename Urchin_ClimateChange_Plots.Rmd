...
```{r Environmental Plots: Temperature, echo = FALSE}
...
ggplot(aes(Day, meanT, color = Temperature)) +
...
```{r Environmental Plots: pH, echo = FALSE}
...
ggplot(aes(Day, meanpH, color = pH)) +
    geom_point(size = 2.5, show.legend=FALSE) +
...
```{r growth by treatment, echo = FALSE}
...
results %>% 
  ...
  ggplot(aes(x=pH, y=mean, color = Temperature, group=interaction(Temperature,pH))) +
    geom_point(size = 3,  position=position_dodge(0.1)) +
    geom_line(aes(group=Temperature),  position=position_dodge(0.1)) +
    xlab("pH Treatment") +
    ylab("Total Growth (%)")+
    geom_errorbar(aes(ymin = mean-se, ymax = mean+se, width = 0.1), position=position_dodge(0.1)) +
    theme_bw() + 
    scale_x_discrete(limits=c("ambient", "acidified"), 
                     label=c("Ambient", "Acidified"))+
...
results %>% 
  ...
  ggplot(aes(x=Temperature, y=mean, color = pH, group=interaction(Temperature,pH))) +
    geom_point(size = 3,  position=position_dodge(0.1)) +
...
```{r Temperature Growth, echo = FALSE}
...
```{r calcification ratio TxpH, echo = FALSE}
...
results %>% 
  select("Temperature","pH","RatioSEM", "PartOfSpine") %>%
  drop_na(RatioSEM) %>% 
  group_by(Temperature, pH, PartOfSpine) %>%
  summarize(mean = mean(RatioSEM), se = std.error(RatioSEM)) %>%

  ggplot(aes(x=Temperature, y=mean, color = pH, group=interaction(Temperature,pH))) +
    facet_wrap(~PartOfSpine)+
    geom_point(size = 3,  position=position_dodge(0.1)) +
    geom_line(aes(group=pH),  position=position_dodge(0.1)) +
    xlab("Temperature Treatment") +
    ylab("Calcification Ratio")+
    geom_errorbar(aes(ymin = mean-se, ymax = mean+se, width = 0.1), position=position_dodge(0.1)) +
    theme_bw() + 
    scale_x_discrete(limits=c("ambient", "heated"), 
                     label=c("Ambient", "High"))+
...
results %>% 
  ...
  ggplot(aes(x=pH, y=mean, color = Temperature, group=interaction(Temperature,pH))) +
    ...
    scale_x_discrete(limits=c("ambient", "acidified"), 
                     label=c("Ambient", "Acidified"))+
    scale_colour_manual(name = "Temperature Treatment",
                        values = c("gray40", "firebrick3"),
                        labels = c("Ambient", "High")) +
    #ylim(250,450)+
    theme_classic()+
...
### Effect on Relative Spine Length
...
```{r Relative Spine Lengths, echo = FALSE, warning=FALSE}
...
results %>% 
  ...
  filter(Chosen == "yes") %>% 
  group_by(Temperature, pH) %>%
  summarize(mean = mean(Ratio), se = std.error(Ratio)) %>%

  ggplot(aes(x=Temperature, y=mean, color = pH, group=interaction(Temperature,pH))) +
    geom_point(size = 3,  position=position_dodge(0.1)) +
    geom_line(aes(group=pH),  position=position_dodge(0.1)) +
    xlab("Temperature Treatment") +
    ylab("Relative Spine Lengths")+
    geom_errorbar(aes(ymin = mean-se, ymax = mean+se, width = 0.1), position=position_dodge(0.1)) +
    ylim(10,20) +
    theme_bw() + 
    scale_x_discrete(limits=c("ambient", "heated"), 
...
results %>% 
  ...
  drop_na(SpineCount) %>% 
  group_by(Temperature, pH) %>%
  summarize(mean = mean(SpineCount), se = std.error(SpineCount)) %>%
...
```{r Dropped Spine by Treatment, echo = FALSE}
...
```{r Spine Steroem Organization, echo = FALSE}
...
structure %>% 
  ...
  ggplot(aes(x=pH, y=PcntUnorganized, color = Temperature, group=interaction(Temperature,pH))) +
    geom_point(size = 3,  position=position_dodge(0.1)) +
    geom_line(aes(group=Temperature),  position=position_dodge(0.1)) +
    xlab("pH Treatment") +
    ylab("Percent Unorganized")+
