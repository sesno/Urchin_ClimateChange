...
3. Response Analyses:
    ...
    b. Calcification Ratio
        ...
        iii. Figure 7: Calcification Interaction Plots
    c. Spine Loss (count)
        i.   Spine Loss Analysis
        ii.  Figure 8: Spine Loss Interaction Plot
...
```{r plotgrid Temp and pH, results=FALSE, echo=FALSE, warning=FALSE, message=FALSE}

#Make plots in vertical line with proper labeling of a and b

treatmentfig1<-plot_grid(tempplot, pHplot, labels = c("a", "b"), ncol=1, nrow=2, rel_heights= c(1,1), rel_widths = c(1,1), label_size = 20, label_y=1, align="h");treatmentfig1

ggsave(filename="Figures/20200611/TreatmentFig1.png", plot=treatmentfig1, dpi=300, width=8, height=8, units="in")
...
### b. Results of ANOVA treatment effect on diameter (day 1)
```{r, message=FALSE, warning=FALSE}

#Prep data for ANOVA diameter comparisons at InitialDiameter1 (day -24, collection), AcclimatedDiameter1 (day -13 after in ambient for ~2wks), InitialDiameter1 (day 1 of experiment, after another 2wks ramp up), and Day126Diameter1 (final diameters)

#Initial Diameter (day -24)
InitialDiameter1 <-
  results %>% 
    select("Day", "Diam1", "Diam2", "TankID", "Temperature", "pH", "Treatment") %>%
    filter(TankID != "8t" ) %>% 
...
Day1Diameter1 <-
  results %>% 
    ...
    drop_na(Diam1, Diam2) %>% 
    mutate(Diameter = ((Diam1+Diam2)/2)) %>% 
      #average the two diameter measurements
    filter(Day == "1") %>% 
      #sizes on day 1  
    select("TankID", "Diameter", "Temperature", "pH", "Treatment")
...
##Calculate all ANOVAs
summary(aov(Diameter ~ Treatment, data=InitialDiameter1))
summary(aov(Diameter ~ Treatment, data=AcclimatedDiameter1))
summary(aov(Diameter ~ Treatment, data=Day1Diameter1))
summary(aov(Diameter ~ Treatment, data=Day126Diameter1))
...
```{r plotgrid Temp and pH Growth, results=FALSE, echo=FALSE, warning=FALSE, message=FALSE}

#Make plots in vertical line with proper labeling of a and b

GrowthFigs<-plot_grid(GrowthTemp, GrowthpH, labels = c("a", "b"), ncol=1, nrow=2, rel_heights= c(1,1), rel_widths = c(1,1), label_size = 15, label_y=1, align="h");GrowthFigs

ggsave(filename="Figures/20200611/GrowthFigs.png", plot=GrowthFigs, dpi=400, width=6, height=8, units="in")
...
calcificationfig1<-plot_grid(tipplot, baseplot, labels = c("a", "b"), ncol=1, nrow=2, rel_heights= c(1,1), rel_widths = c(0.8,1), label_size = 20, label_y=1.01, align="h");calcificationfig1

ggsave(filename="Figures/20200611/CalcificationFig1_2.png", plot=calcificationfig1, dpi=300, width=6, height=8, units="in")

```

#### iiib. Alternate Figure 4: Calcification Ratio bar plot at tips and bases
```{r}

calcificationBarPlot <-
results %>% 
  select("Temperature","pH","RatioSEM", "PartOfSpine") %>%
  drop_na(RatioSEM) %>% 
  ...
  ggplot(aes(Temperature, mean, fill = pH)) +
    ...
    scale_y_continuous(name="Calcification Ratio", 
    ...
    theme(panel.grid.major = element_blank(),
          ...
          legend.justification = c("right", "top"),
          legend.background = element_blank()) +
    geom_text(aes(x= 1, y = 1, label = "")) ; calcificationBarPlot
  

ggsave(filename="Figures/20200611/CalcPlotBar.png", plot=calcificationBarPlot, dpi=300, width=7, 
       height=5, units="in")
```
